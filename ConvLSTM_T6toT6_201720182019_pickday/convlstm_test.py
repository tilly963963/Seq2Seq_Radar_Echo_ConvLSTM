...
#                trainer.train(model, ims, real_input_flag, args, itr, index ,num_of_batch_size, save_path, model_name)
        ...
        load_model=False
        test_cost = val(model, save_path, model_pkl, itr,val_radar_xy_shuffle=None)
...
def test_show(model, save_path, model_name,itr):
    ...
    save_path = save_path + 'test_show_itr_{}_{}_Sun_Moon_Lake_dbz1_csi_testcsi_v2/'.format(itr,data_name)
    from pickle import load
    # places=['Bao_Zhong']
...
# 
    ...
    # date_date=[['2020-05-19 00:00', '2020-05-19 00:09']]
    # test_date=[['2020-05-19 00:00', '2020-05-19 00:09']]
    ...
    for place in places: 
        ...
        for index in range(num_of_batch_size):
            ...
            # a=5
            # a= torch.tensor([5])
            # img_out2 = np.array(img_out[:,-1].reshape(512*512))
            # img_out[:,-1]= torch.where(img_out2 >= 43, img_out2*0.9, img_out2).reshape(512,512)
            # for i in img_out[:,-1]:
            ...
            #             if  img_out[:, h, i, k, :]>40:
            #                 img_out[:, h, i, k, :] = img_out[:, -1, i, k, :]*0.99
...
if __name__ == "__main__":                
    ...
    date_date=[['2017-04-21 10:00', '2017-04-21 17:00'],
                ...
                ['2018-08-23 00:00', '2018-08-23 21:00'],
                ['2018-08-24 04:00', '2018-08-24 08:00'],
    ...
    # date_date = [['2018-08-24 00:00','2018-08-24 00:59']]

    # test_date=[['2018-08-24 00:10', '2018-08-24 00:19']]
    ...
    #          ['2020-05-27 00:00','2020-05-27 23:59']]
    test_date = [['2018-08-23 22:00','2018-08-24 04:00']]
   
    # val_data=[['2018-08-24 00:10', '2018-08-24 23:59'],#]#,#,#]#,
    #          ['2020-05-16 00:00','2020-05-16 23:59'],
    #          ['2020-05-19 00:00','2020-05-19 23:59'],
    #          ['2020-05-21 00:00','2020-05-21 23:59'],
    #          ['2020-05-22 00:00','2020-05-22 23:59'],
    ...
    radar_echo_storage_path= 'NWP/'#'NWP/'
    load_radar_echo_df_path='ConvLSTM_T6toT6_201720182019_pickday/2017to2018_pick_day.pkl'#'online_test/T12toT6_test_512x512_.pkl'
    
    save_path ='ConvLSTM_T6toT6_201720182019_pickday/'
    ...
    # if not load_radar_echo_df_path:
        ...
        # if not os.path.isdir(save_path):
            # os.makedirs(save_path)
        # radar.saveRadarEchoDataFrame(path=save_path ,load_name_pkl='T12toT6_201808_pickday_512x512')   
    ...
    #    scaler = StandardScaler()
        ...
        test_show(model, save_path, model_name,itr=20)
